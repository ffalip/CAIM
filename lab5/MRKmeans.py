    ...
    for i in range(args.iter):
        ...
        with mr_job1.make_runner() as runner1:
            ...
            for key, value in mr_job1.parse_output(runner1.cat_output()):
                ...
                new_assign[key] = value[0] #llista de docs
                new_proto[key] = value[1] #llista de termes

            if new_assign == assign:
                nomove = True
            # If your scripts returns the new assignments you could write them in a file here
            strAssign = ""
            for clusterID, docs in new_assign.items():
                strAssign += clusterID + ":"
                for doc in docs:
                    strAssign += doc + " "
                strAssign += "\n"

            assignFile = "/assignments%d.txt" % (i+1)
            if (i+1 == args.iter or nomove):
                assignFile = "/assignments-final.txt"
            outputAssign = open(cwd + assignFile, 'w')
            outputAssign.write(strAssign)
            outputAssign.close()

            strPrototypes = ""
            for clusterID, terms in new_proto.items():
                strPrototypes += clusterID + ":"
                for term in terms:
                    strPrototypes += term[0] + "+" + str(term[1]) + " "
                strPrototypes += "\n"
            # You should store the new prototypes here for the next iteration
            prototypeFile = "/prototypes%d.txt" % (i+1)
            if (i+1 == args.iter or nomove):
                prototypeFile = "/prototypes-final.txt"
            outputPrototypes = open(cwd + prototypeFile, 'w')
            outputPrototypes.write(strPrototypes)
            outputPrototypes.close()
